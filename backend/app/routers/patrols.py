...
@router.get("/", response_model=list[PatrolRead], dependencies=[Depends(require(Permission.MEMBER_READ))])
...
@router.post("/", response_model=PatrolRead, status_code=201, dependencies=[Depends(require(Permission.MEMBER_WRITE))])
...
@router.get("/{patrol_id}", response_model=PatrolRead, dependencies=[Depends(require(Permission.MEMBER_READ))])
...
@router.patch("/{patrol_id}", response_model=PatrolRead, dependencies=[Depends(require(Permission.MEMBER_WRITE))])
def update_patrol(
...
@router.delete("/{patrol_id}", status_code=204, dependencies=[Depends(require(Permission.MEMBER_DELETE))])
