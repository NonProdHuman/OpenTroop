...
def _get_or_404(db, assignment_id: uuid.UUID, tenant_id: uuid.UUID) -> MemberRoleAssignment:
...
def _check_role_tenant(db, role_id: uuid.UUID, tenant_id: uuid.UUID) -> None:
...
def create_role_assignment(body: MemberRoleAssignmentBase, tenant_id: TenantDep, db: DbDep):
...
def get_role_assignment(assignment_id: uuid.UUID, tenant_id: TenantDep, db: DbDep):
    return _get_or_404(db, assignment_id, tenant_id)


@router.delete("/{assignment_id}", status_code=204)
def delete_role_assignment(assignment_id: uuid.UUID, tenant_id: TenantDep, db: DbDep):
