...
def _get_or_404(db, member_id: uuid.UUID, tenant_id: uuid.UUID) -> Member:
...
def _check_patrol_tenant(db, patrol_id: uuid.UUID, tenant_id: uuid.UUID) -> None:
...
def list_members(tenant_id: TenantDep, db: DbDep):
    return db.scalars(
...
def create_member(body: MemberBase, tenant_id: TenantDep, db: DbDep):
...
def get_member(member_id: uuid.UUID, tenant_id: TenantDep, db: DbDep):
    return _get_or_404(db, member_id, tenant_id)


@router.patch("/{member_id}", response_model=MemberRead)
def update_member(member_id: uuid.UUID, body: MemberUpdate, tenant_id: TenantDep, db: DbDep):
...
def delete_member(member_id: uuid.UUID, tenant_id: TenantDep, db: DbDep):
    member = _get_or_404(db, member_id, tenant_id)
