...
# The feed looks members up by the *hash* of the presented token (GH-114).
_LOOKUP = text("SELECT id FROM members WHERE calendar_token_hash = :t AND is_deleted = false")
...
def test_feed_token_lookup_requires_tenant_guc(pg_engine: Engine, app_conn) -> None:  # type: ignore[type-arg]
    ...
    tenant_id = uuid7()
    member_id = uuid7()
    token_hash = hashlib.sha256(f"feed-{uuid7().hex}".encode()).hexdigest()
    ...
    with pg_engine.connect() as conn:
        ...
        conn.execute(
            text(
                ...
                INSERT INTO members
                  (id, tenant_id, first_name, last_name, member_type,
                   ...
                   email_opt_out, email_bounced, sms_opt_in, calendar_token_hash,
            ...
            {"id": str(member_id), "tid": str(tenant_id), "token": token_hash},
        )
        conn.commit()

    try:
        # BUG path — what the un-scoped feed did: no tenant GUC ⇒ RLS hides the row ⇒ 404.
        assert app_conn.execute(_LOOKUP, {"t": token_hash}).first() is None
        ...
        assert app_conn.execute(_LOOKUP, {"t": token_hash}).first() is not None
    finally:
        with pg_engine.connect() as conn:
